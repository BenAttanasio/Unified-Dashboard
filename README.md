...
dark, kiosk-friendly screen. Built with Next.js, so it runs anywhere Node runs.
It was designed for a Raspberry Pi kiosk and ports to serverless hosts.
...
- **Cost.** A monthly Apify usage tracker showing spend against the plan cap
...
rest carry on working. See [`.env.example`](.env.example) for all variables. A status page
...
- All credentials stay in `.env`, which is gitignored, so nothing secret is
