...
/**
 * The snapshot value nearest to `aroundMs` ago. With `toleranceMs` set, returns
 * null unless the nearest snapshot actually falls within that window — use it
 * when "no history that far back yet" must read as unknown rather than as the
 * newest value (which would silently report a growth of zero).
 */
export function getValueAt(
  ...
  const row = handle
    ...
    .get(platform, metricName, targetIso) as { value: number; recorded_at: string } | undefined;
  if (!row) return null;
  if (toleranceMs != null) {
    // SQLite stores timestamps as "YYYY-MM-DD HH:MM:SS" in UTC.
    const ts = Date.parse(row.recorded_at.replace(" ", "T") + "Z");
    if (!Number.isFinite(ts) || Math.abs(ts - (Date.now() - aroundMs)) > toleranceMs) return null;
  }
  return row.value;
}

/**
 * The OLDEST snapshot within the last `sinceMs`, with its age in ms. Lets a
 * growth figure fall back to "change over however much history exists" instead
 * of showing nothing at all for the first week of a newly tracked source.
 */
export function getEarliestSince(
  platform: string,
  metricName: string,
  sinceMs: number,
  ...
  const row = handle
    ...
    .get(platform, metricName, sinceIso) as { value: number; recorded_at: string } | undefined;
  if (!row) return null;
  const ts = Date.parse(row.recorded_at.replace(" ", "T") + "Z");
  if (!Number.isFinite(ts)) return null;
  return { value: row.value, ageMs: Math.max(0, Date.now() - ts) };
}
...
export function getDelta(
  ...
  const past = getValueAt(platform, metricName, aroundMs);
  return past == null ? null : currentValue - past;
}
